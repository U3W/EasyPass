...
pub fn generate_new_factor(password: &str, master_key: &[u8]) -> (String, String) { //new_encrypted_key (save to db), string to put into file
    ...
    let key = passwords::password_to_key(password, iv);
    let encrypted_secret_1 = Statisch::encrypt_secret(priv_key_1, key.as_bytes());
    let encrypted_secret_2 = Statisch::encrypt_secret(priv_key_2, key.as_bytes());
    iv.push_str("§");
...
pub fn two_factors_to_key(password: &str, encrypted_master_key: &str, second_factor: &str) -> Vec<u8>{
    ...
    let key = passwords::password_to_key(password, String::from(iv));
